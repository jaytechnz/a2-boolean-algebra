...
/* ══════════════════════════════════════════
   ...
   Two input methods:
   1. SYMBOL STRIP — tappable buttons (always works)
   2. DRAWING PAD  — uses the $P Point-Cloud Recognizer
      (Vatavu et al., ICMI 2012) for ML-based gesture
      recognition. Handles multi-stroke symbols: letters,
      operators, digits, brackets. No network needed.
   ══════════════════════════════════════════ */

/* ── $P Point-Cloud Recognizer ──
   Reference: R.-D. Vatavu, L. Anthony, J.O. Wobbrock.
   "Gestures as Point Clouds." Proc. ICMI 2012.
   Adapted for Boolean algebra symbol recognition.       */

const PointCloud = {
  N: 32,          // Resample to N points
  ORIGIN: { x: 0, y: 0 },

  /** Create a named template from raw strokes */
  makeTemplate(name, strokes) {
    let pts = [];
    strokes.forEach((stroke, sid) => {
      stroke.forEach(p => pts.push({ x: p.x, y: p.y, id: sid }));
    });
    pts = this.resample(pts, this.N);
    pts = this.scale(pts);
    pts = this.translate(pts, this.ORIGIN);
    return { name, points: pts };
  },

  /** Recognise drawn strokes against a set of templates */
  recognise(strokes, templates) {
    let pts = [];
    strokes.forEach((stroke, sid) => {
      stroke.forEach(p => pts.push({ x: p.x, y: p.y, id: sid }));
    });
    if (pts.length < 2) return { name: '·', score: 1 }; // Tap = dot
    ...
    for (const tmpl of templates) {
      const d = this.greedyCloudMatch(pts, tmpl.points);
      if (d < bestD) { bestD = d; bestName = tmpl.name; }
    }

    // Score: 0 (bad) to 1 (perfect)
    const score = Math.max(0, 1 - bestD / (0.5 * Math.sqrt(2)));
    ...
    for (let i = 1; i < points.length; i++) {
      if (points[i].id === points[i - 1].id) {
        ...
        if (D + d >= interval) {
          ...
          while (D + remaining >= interval && resampled.length < n) {
            const t = (interval - D) / remaining;
            const nx = px + t * (points[i].x - px);
            const ny = py + t * (points[i].y - py);
            resampled.push({ x: nx, y: ny, id: points[i].id });
            remaining -= (interval - D);
            D = 0;
            px = nx; py = ny;
          }
          D = remaining;
  ...
  translate(points, origin) {
    ...
    return points.map(p => ({ x: p.x - cx + origin.x, y: p.y - cy + origin.y, id: p.id }));
  },

  greedyCloudMatch(pts1, pts2) {
    const n = pts1.length;
    const eps = 0.5;
    let step = Math.floor(Math.pow(n, 1 - eps));
    if (step < 1) step = 1;
    let minD = Infinity;
  ...
  cloudDistance(pts1, pts2, startIdx) {
    const n = pts1.length;
    ...
    for (let k = 0; k < n; k++) {
      ...
      if (minIdx >= 0) matched[minIdx] = true;
      sum += Math.sqrt(minD);
    }
    return sum / n;
  },
...
/* ── Template Definitions ──
   Each symbol has multiple hand-drawn style variants.
   Points are normalised — the $P algorithm handles
   scale/translation invariance automatically.          */

function generateTemplates() {
  const T = [];
  const add = (name, ...strokeArrays) => T.push(PointCloud.makeTemplate(name, strokeArrays));

  // Helper: generate points along a line
  const line = (x1, y1, x2, y2, n = 20) => {
    const pts = [];
    for (let i = 0; i < n; i++) { const t = i / (n - 1); pts.push({ x: x1 + t * (x2 - x1), y: y1 + t * (y2 - y1) }); }
    return pts;
  };

  // Helper: generate points along an arc
  const arc = (cx, cy, rx, ry, startA, endA, n = 20) => {
    const pts = [];
    for (let i = 0; i < n; i++) { const a = startA + (endA - startA) * i / (n - 1); pts.push({ x: cx + rx * Math.cos(a), y: cy + ry * Math.sin(a) }); }
    return pts;
  };

  // ── PLUS (+) — two-stroke cross ──
  add('+', line(10, 50, 90, 50), line(50, 10, 50, 90));
  add('+', line(15, 50, 85, 50), line(50, 15, 50, 85));
  // Single-stroke plus (drawn without lifting)
  add('+', [...line(10, 50, 90, 50), ...line(50, 10, 50, 90)]);
  add('+', [...line(50, 10, 50, 90), ...line(10, 50, 90, 50)]);
  // Slightly off-centre variants
  add('+', line(10, 55, 90, 45), line(45, 10, 55, 90));
  add('+', line(10, 48, 90, 52), line(48, 10, 52, 90));

  // ── OVERBAR / NOT (') — horizontal line ──
  add("'", line(10, 50, 90, 50));
  add("'", line(5, 50, 95, 50));
  add("'", line(10, 45, 90, 55));  // Slight slant
  add("'", line(10, 55, 90, 45));
  add("'", line(15, 50, 85, 50));

  // ── DOT / AND (·) — very small tap (handled separately) ──

  // ── VERTICAL LINE / 1 ──
  add('1', line(50, 10, 50, 90));
  add('1', line(50, 5, 50, 95));
  add('1', line(48, 10, 52, 90));
  add('1', line(45, 10, 55, 90));

  // ── ZERO (0) — circle ──
  add('0', arc(50, 50, 35, 40, 0, Math.PI * 2));
  add('0', arc(50, 50, 30, 35, 0, Math.PI * 2));
  add('0', arc(50, 50, 25, 40, 0.1, Math.PI * 2 + 0.1));
  add('0', arc(50, 50, 35, 35, -0.2, Math.PI * 2 - 0.2));
  // Tall narrow ovals
  add('0', arc(50, 50, 20, 40, 0, Math.PI * 2));
  add('0', arc(50, 50, 18, 38, 0.1, Math.PI * 2 + 0.1));
  add('0', arc(50, 50, 22, 42, -0.1, Math.PI * 2 - 0.1));
  // Wide flat ovals
  add('0', arc(50, 50, 40, 25, 0, Math.PI * 2));
  add('0', arc(50, 50, 38, 22, 0.2, Math.PI * 2 + 0.2));
  // Starting from top
  add('0', arc(50, 50, 30, 38, -Math.PI / 2, Math.PI * 1.5));
  add('0', arc(50, 50, 28, 35, -Math.PI / 2, Math.PI * 1.5));

  // ── OPEN PAREN ( ──
  add('(', arc(70, 50, 35, 40, Math.PI * 0.6, Math.PI * 1.4));
  add('(', arc(65, 50, 30, 40, Math.PI * 0.55, Math.PI * 1.45));
  add('(', arc(70, 50, 25, 35, Math.PI * 0.65, Math.PI * 1.35));

  // ── CLOSE PAREN ) ──
  add(')', arc(30, 50, 35, 40, -Math.PI * 0.4, Math.PI * 0.4));
  add(')', arc(35, 50, 30, 40, -Math.PI * 0.45, Math.PI * 0.45));
  add(')', arc(30, 50, 25, 35, -Math.PI * 0.35, Math.PI * 0.35));

  // ── EQUALS (=) — two horizontal lines ──
  add('=', line(10, 35, 90, 35), line(10, 65, 90, 65));
  add('=', line(15, 30, 85, 30), line(15, 70, 85, 70));
  add('=', line(10, 38, 90, 38), line(10, 62, 90, 62));

  // ── LETTER A ──
  // Two-stroke: /\ shape
  add('A', line(10, 90, 50, 10), line(50, 10, 90, 90));
  // Three-stroke: /\ + crossbar
  add('A', line(10, 90, 50, 10), line(50, 10, 90, 90), line(25, 55, 75, 55));
  // Single-stroke triangular A
  add('A', [...line(10, 90, 50, 10), ...line(50, 10, 90, 90), ...line(70, 55, 30, 55)]);
  // Variants
  add('A', line(15, 85, 50, 15), line(50, 15, 85, 85));
  add('A', line(5, 90, 50, 5), line(50, 5, 95, 90), line(20, 55, 80, 55));

  // ── LETTER B ──
  // Vertical + two bumps
  add('B', line(20, 10, 20, 90), [...arc(20, 30, 30, 20, -Math.PI / 2, Math.PI / 2), ...arc(20, 70, 30, 20, -Math.PI / 2, Math.PI / 2)]);
  // Single stroke B-shape
  add('B', [...line(20, 90, 20, 10), ...arc(20, 30, 28, 20, -Math.PI / 2, Math.PI / 2), ...arc(20, 70, 32, 20, -Math.PI / 2, Math.PI / 2)]);
  add('B', [...line(25, 85, 25, 15), ...arc(25, 35, 25, 20, -Math.PI / 2, Math.PI / 2), ...arc(25, 65, 28, 20, -Math.PI / 2, Math.PI / 2)]);

  // ── LETTER C ──
  add('C', arc(60, 50, 35, 40, Math.PI * 0.4, Math.PI * 1.6));
  add('C', arc(55, 50, 30, 40, Math.PI * 0.35, Math.PI * 1.65));
  add('C', arc(60, 50, 35, 35, Math.PI * 0.45, Math.PI * 1.55));

  // ── LETTER D ──
  add('D', line(20, 10, 20, 90), arc(20, 50, 40, 40, -Math.PI / 2, Math.PI / 2));
  add('D', [...line(25, 90, 25, 10), ...arc(25, 50, 35, 40, -Math.PI / 2, Math.PI / 2)]);
  add('D', line(20, 15, 20, 85), arc(20, 50, 38, 38, -Math.PI / 2, Math.PI / 2));

  // ── LETTER E ──
  add('E', [...line(70, 10, 20, 10), ...line(20, 10, 20, 90), ...line(20, 90, 70, 90)], line(20, 50, 60, 50));
  add('E', line(20, 10, 20, 90), line(20, 10, 70, 10), line(20, 50, 60, 50), line(20, 90, 70, 90));
  // Single-stroke E
  add('E', [...line(65, 15, 20, 15), ...line(20, 15, 20, 50), ...line(20, 50, 55, 50), ...line(55, 50, 20, 50), ...line(20, 50, 20, 85), ...line(20, 85, 65, 85)]);
...
const BooleanInput = {
  ...
  initCanvas() {
    const canvas = document.getElementById('bool-draw-pad');
    if (!canvas) return;

    // Build templates on first init
    if (!this._templates) this._templates = generateTemplates();
    ...
    const endStroke = () => {
      ...
      this._currentStroke = null;
      clearTimeout(this._recogniseTimer);
      this._recogniseTimer = setTimeout(() => this._recognise(), 700);
    };
  ...
  clearCanvas() {
    ...
    this._strokes = [];
    clearTimeout(this._recogniseTimer);
    const label = document.getElementById('draw-recognised');
  ...
  _recognise() {
    ...
    // Tiny tap = dot (AND)
    const allPts = this._strokes.flat();
    let minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;
    ...
    if ((maxX - minX) < 12 && (maxY - minY) < 12) {
      if (label) { label.textContent = 'AND (·)'; label.classList.add('flash'); setTimeout(() => label.classList.remove('flash'), 400); }
      this.insert('·');
      setTimeout(() => this.clearCanvas(), 300);
      return;
    }

    // $P recognition
    const result = PointCloud.recognise(this._strokes, this._templates);

    if (result.score > 0.3) {
      const names = { "'": 'NOT (overbar)', '+': 'OR (+)', '·': 'AND (·)', '=': 'equals', '(': '(', ')': ')', '0': '0', '1': '1' };
      if (label) {
        label.textContent = (names[result.name] || result.name) + ' ' + Math.round(result.score * 100) + '%';
        label.classList.add('flash');
        setTimeout(() => label.classList.remove('flash'), 400);
      }
      this.insert(result.name);
      setTimeout(() => this.clearCanvas(), 300);
    } else {
      if (label) label.textContent = '? (' + Math.round(result.score * 100) + '%) try again';
    }
  }
};
  ...
  renderExercise() {
    ...
    area.innerHTML = `
      ...
      <div class="exercise-card">
        ...
        <div class="bool-draw-section">
          <div class="bool-draw-header">
            <span class="bool-draw-title">✏️ Draw any symbol</span>
            <span class="bool-draw-recognised" id="draw-recognised"></span>
            <button class="btn btn-secondary btn-sm" onclick="BooleanInput.clearCanvas()" style="padding:2px 10px;font-size:0.7rem;">Clear</button>
          </div>
          <canvas id="bool-draw-pad" class="bool-draw-pad"></canvas>
          <div class="bool-draw-hint">Draw letters (A–E), operators (<strong>+</strong> <strong>·</strong> <strong>overbar</strong>), digits (0, 1), or brackets. Tap for AND (·). Recognised after a short pause.</div>
        </div>
      </div>
    ...
    // Start input polling + init drawing pad + track focus
